...
//*************************************************************************************
so_geo::result plane::make( polygon_mesh_ptr_t mesh_ptr, input_params_cref_t ip ) 
{
    if( so_core::is_nullptr(mesh_ptr) )
    ...
    so_geo::polygon_mesh_ref_t m = *mesh_ptr ;
...
void_t plane::construct_vertices( data_ref_t d, input_params_cref_t ip ) 
{
    ...
    // vertices
    {
        ...
        for(size_t iy = 0; iy < per_dim_verts; ++iy)
        {
            ...
            for(size_t ix = 0; ix < per_dim_verts; ++ix)
            {
                ...
                so_math::vec3f_t v{ x*ip.scale.x(), y*ip.scale.y(), 0.0f } ;                
                so_math::vec2f_t t{ tx, ty } ;

                size_t const i3d = index * 3 ;
                size_t const i2d = index * 2 ;

                so_3d::helper::vector_to_array( i3d, v, d.positions ) ;                
                so_2d::helper::vector_to_array( i2d, t, d.texcoords[0] ) ;
    ...
    // normals
    {
        ...
        so_math::vec3f_t n{ 0.0f, 0.0f, -1.0f } ;
        so_3d::helper::vector_to_array( 0, n, d.normals ) ;
    }
}
