...
//------------------------------------------------------------
#include "gl33_circle_render_2d_plug.h"
...
//************************************************************************************
gl33_circle_render_2d_plug::gl33_circle_render_2d_plug( so_gfx::circle_render_2d_shared_data_ptr_t sd_ptr,
...
//************************************************************************************
gl33_circle_render_2d_plug::gl33_circle_render_2d_plug( this_rref_t rhv ) : this_base_t( std::move( rhv ) )
{
...
//************************************************************************************
gl33_circle_render_2d_plug::~gl33_circle_render_2d_plug( void_t )
{
...
//************************************************************************************
gl33_circle_render_2d_plug::this_ptr_t gl33_circle_render_2d_plug::create( this_rref_t rhv, so_memory::purpose_cref_t p )
{
    return so_gfx::memory::alloc( std::move( rhv ), p ) ;
}

//************************************************************************************
void_t gl33_circle_render_2d_plug::destroy( this_ptr_t ptr )
{
...
so_gpx::plug_result gl33_circle_render_2d_plug::on_load( load_info_cref_t li )
{
    ...
    auto const base = so_gfx::find_shader_path( "/circle/gl33/shader" ) ;

    // text render shader
    {
        auto const vs_path = base / so_io::path_t( "/circle_render_2d.vs.glsl" ) ;
        auto const ps_path = base / so_io::path_t( "/circle_render_2d.ps.glsl" ) ;
...

        {
            ...
            vsh.wait_for_operation( [&] ( char_cptr_t din, size_t sib, so_io::result res )
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_circle_render_2d_plug::on_load] : Vertex shader not found : " + vs_path.string() ) ;
...

        {
            ...
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_circle_render_2d_plug::on_load] : Pixel shader not found : " + ps_path.string() ) ;
...

    {
        ...
        size_t const num_circ_verts = _num_circ_verts ;
        float_t const step_rad = 2.0f * 3.143234f / float_t( num_circ_verts ) ;

        _vb->resize( num_circ_verts * num_rects ) ;

        // init first circle
        {
            for( size_t cp = 0; cp < num_circ_verts; ++cp )
            {
                _vb->set_vertex( cp, vertex_t { so_math::vec3f_t(
                    std::cos( float_t( cp )*-step_rad ), std::sin( float_t( cp )*-step_rad ), 0.0f ) } ) ;
            }
        }

        // copy all other from first circle
        {
            size_t base_i = 0 ;
            ...
            for( size_t i = 0; i < num_rects-1; ++i )
            {
                for( size_t cp = 0; cp < num_circ_verts; ++cp )
                {
                    _vb->set_vertex( base_i + cp, _vb->get_vertex( cp ) ) ;
                }
                base_i += num_circ_verts ;
            }
        }

        uint_t const num_tris = uint_t( num_circ_verts ) - 2 ;
        _ib->reserve( num_tris * 3 ) ;

        uint_t base_i = 0 ;
        for( size_t i = 0; i < num_rects; ++i )
        {
            for( uint_t j = 0; j < num_tris; ++j )
            {
                _ib->add_index( base_i ) ;
                _ib->add_index( base_i + j + 1 ) ;
                _ib->add_index( base_i + j + 2 ) ;
            }

            base_i += uint_t(num_circ_verts) ;
        }
    }

    {
        ...
        _rect_info_ptr = data_buffer_t::create( std::move( db ),
            "[gl33_circle_render_2d_plug::on_load] : rect_info" ) ;
    }
...
//************************************************************************************
so_gpx::plug_result gl33_circle_render_2d_plug::on_unload( void_t )
{
    return so_gpx::plug_result::ok ;
}

//************************************************************************************
so_gpx::plug_result gl33_circle_render_2d_plug::on_initialize( init_info_cref_t )
{
    {
        {
...
//************************************************************************************
so_gpx::plug_result gl33_circle_render_2d_plug::on_release( void_t )
{
...
//************************************************************************************
so_gpx::plug_result gl33_circle_render_2d_plug::on_update( update_info_cref_t )
{
    // 1. copy the data
    {
            ...
            for( auto const & gi : _sd->per_group_infos )
            {
                ...
                if( iter == _per_group_datas.end() )
                {
                    ...
                    auto * var_set = so_gpu::variable_set_t::create(
                        "[gl33_circle_render_2d_plug::on_update] : variable_set" ) ;
...
//************************************************************************************
so_gpx::plug_result gl33_circle_render_2d_plug::on_execute( execute_info_cref_t ri )
{
    auto iter = std::find_if( _per_group_datas.begin(), _per_group_datas.end(),
    ...
    if( iter == _per_group_datas.end() )
    {
        so_log::global::warning( "[gl33_circle_render_2d_plug::on_execute] : "
    ...
    this_t::api()->set_state( so_gpu::blend_factor::one, so_gpu::blend_factor::one_minus_src_alpha ) ;
    {
        ...
        this_t::api()->execute( so_gpu::render_config_info( _config, iter->varset_id, 0,
            ( iter->num_elements ) * (_num_circ_verts-2)*3 ) ) ;
    }
...
//************************************************************************************
void_t gl33_circle_render_2d_plug::destroy( void_t )
{
    this_t::destroy( this ) ;
}
