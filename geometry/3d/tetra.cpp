...
//*************************************************************************************
so_geo::result tetra::make( polygon_mesh_ptr_t m_out_ptr, input_params_cref_t ip ) 
{
    if( so_core::is_nullptr(m_out_ptr) )
        return so_geo::invalid_argument ;

    so_geo::polygon_mesh m ;
    
    m.position_format = so_geo::vector_component_format::xyz ;
    m.normal_format = so_geo::vector_component_format::xyz ;
    m.texcoord_format = so_geo::texcoord_component_format::uv ;

    so_math::vec3f_t const c_positions[] =
    {
        so_math::vec3f_t{ -1.0f, -1.0f, -1.0f }, // bottom left
        so_math::vec3f_t{ 1.0f, -1.0f, -1.0f }, // bottom right
        so_math::vec3f_t{ 0.0f, -1.0f, 1.0f }, // bottom back        
        so_math::vec3f_t{ 0.0f, 1.0f, 0.0f } // tip
    } ;

    so_math::vec3f_t const c_normals[] =
    {
        so_math::vec3f_t{ -0.5f, 0.5f, 0.5f }, // left
        so_math::vec3f_t{ 0.5f, 0.5f, 0.5f }, // right
        so_math::vec3f_t{ 0.0f, 0.5f, -0.5f }, // front
        so_math::vec3f_t{ 0.0f, -1.0f, 0.0f } // bottom
    } ;

    so_math::vec2f_t const c_tex[] =
    {
        so_math::vec2f_t{ +0.0f, +0.0f }, // front 
        so_math::vec2f_t{ +1.0f, +0.0f }, // front 
        so_math::vec2f_t{ +0.0f, +1.0f } // front 
