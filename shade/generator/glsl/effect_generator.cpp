...
//*************************************************************************************
effect_generator::effect_generator( so_shade::so_generator::so_glsl::generator_properties_cref_t props ) 
{
    _gen_props = props ;
}
    ...
    so_shade::so_shader::effect_out_t eff_ptr ) 
{
    ...
    return this_t::generate_glsl33_effect_from( mat_ptr, eff_ptr ) ;
}

//*************************************************************************************
so_shade::result effect_generator::generate_glsl33_effect_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr,
    so_shade::so_shader::effect_out_t eff_out ) 
{
...

    {
        eff.set_vertex_shader( so_shade::so_shader::vertex_shader_t::create(
            ...
            "[so_shade::effect_generator::generate_glsl33_effect_from] : vertex shader" ) ) ;

        so_std::string_t code ;
        code += "// [GEN] : snakeoil glsl 3.30 code generator\n" ;
...

    {
        eff.set_pixel_shader( so_shade::so_shader::pixel_shader_t::create(
            ...
            "[so_shade::effect_generator::generate_glsl33_effect_from] : pixel_shader" ) ) ;
        ...
        code += generate_glsl33_pixel_shader_call_albedo_from() ;
        code += "out_ = albedo_color ; \n" ;
        //code += "out_ = vec4(psi.txc_0.xy,0,0) ; " ;
        code += "\n" ;
        code += this_t::generate_glsl33_linear_z_output_from( mat_ptr ) ;
        code += "}\n" ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_ins_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr,
    so_shade::so_shader::effect_ref_t ein ) 
{
    so_std::string_t expr ;

    expr += "in vec3 "+_gen_props.position_name+" ; \n" ;   
    {
    ...
    {
        expr += "in vec3 "+_gen_props.normal_name+" ; \n" ;
        {
    ...
    for( size_t i=0; i<mat_ptr->get_num_texture_layers(); ++i )
    {
        ...
        so_std::string_t const tc_name = _gen_props.texcoord_pre_name + i_s ;
        expr += "in vec2 " + tc_name + " ;\n" ;
        {
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_uniforms_from(
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_samplers_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr,
    so_shade::so_shader::effect_ref_t ein )
{
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_defines_from( 
    so_shade::so_material::standard_material_ptr_t ) 
{
    so_std::string_t expr ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_texcoord_varyings_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    if( mat_ptr->get_num_texture_layers() == 0 )
    {
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_normal_varyings_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_light_varyings_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    // @todo check lights
    if( so_core::is_not( mat_ptr->has_normal_layer() ) )
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_linear_z_varyings_from(
    so_shade::so_material::standard_material_ptr_t ) 
{
    so_std::string_t expr ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_texcoord_write_through_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    if( mat_ptr->get_num_texture_layers() == 0 )
    {
    ...
    for( size_t i = 0; i < mat_ptr->get_num_texture_layers(); ++i )
    {
        ...
        so_std::string_t const i_s = std::to_string( i ) ;
        expr += "vso.txc_" + i_s + " = ("+ _gen_props.texcoord_pre_name + i_s + " " ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_normal_write_through_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    if( so_core::is_not(mat_ptr->has_normal_layer()) )
    {
...

    {
        ...
        expr += "vso.nrm = vec4( " + view_name + " * " + wrl_name ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_light_write_through_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    if( so_core::is_not( mat_ptr->has_normal_layer() ) )
    {
    ...
    // @todo for each light dir
    {
        ...
        expr += "vso.light_dir = vec4( "+view_name+" * vec4( normalize( to_light ), 0.0 ) ).xyz ;\n" ;
    }
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_linear_z_write_through_from(
    so_shade::so_material::standard_material_ptr_t ) 
{
    so_std::string_t expr ;
    ...
    if( _gen_props.enable_linear_depth )
    {
        ...
        expr += "vso.linear_z = (view_pos.z - __near) / (__far - __near) ; \n " ;
    }
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_texture_access_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    so_std::string_t expr ;
    ...
    for( size_t l=0; l<mat_ptr->get_num_texture_layers(); ++l )
    {
        ...
        for( size_t i=0; i<mat_ptr->get_num_textures(l); ++i )
        {
            ...
            expr += "vec4 "+ var_name + " = texture( " + smp_name + ", psi.txc_"+l_s+" ) ;\n" ;
        }        
    }
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_albedo_output_from(
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    size_t div_parts = 0 ;
    ...
    /*{
        ...
        if( mat_ptr->has_normal_layer() )
        {
            // if has light
            {
                ...
                expr += "float ndl = dot(psi.nrm, normalize(psi.light_dir)) ; \n" ;
                expr += "vec4 light_color = mix( bottom_color, sky_color, ndl ) ; \n\n" ;
                // Test code
                //expr += "out_color = vec4(ndl) ;\n" ;
                //expr += "out_color = vec4(normalize(psi.light_dir),1.0) ;\n" ;
                //expr += "out_color = vec4(psi.nrm,1.0) ;\n" ;
            }
            final_division += 1 ;
        }
    }*/
...

    {
        ...
        //expr = "return vec4(psi.txc_0,0.0,1.0) ;" ;
    }
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_linear_z_output_from( 
    so_shade::so_material::standard_material_ptr_t mat_ptr ) 
{
    so_std::string_t expr ;
    if( _gen_props.enable_linear_depth )
    {
        expr += "gl_FragDepth = psi.linear_z ; \n" ;
    }
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_pixel_shader_call_albedo_from( void_t ) 
{
    so_std::string_t expr ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_obj_to_view_pos_transform_from( 
    so_shade::so_material::standard_material_ptr_t ) 
{
    so_std::string_t expr ;
...
//*************************************************************************************
so_std::string_t effect_generator::generate_glsl33_view_to_proj_pos_transform_from(
    so_shade::so_material::standard_material_ptr_t ) 
{
    so_std::string_t expr ;
