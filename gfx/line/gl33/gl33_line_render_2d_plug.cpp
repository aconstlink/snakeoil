...
//------------------------------------------------------------
#include "gl33_line_render_2d_plug.h"
...
//************************************************************************************
gl33_line_render_2d_plug::gl33_line_render_2d_plug( so_gfx::line_render_2d_shared_data_ptr_t sd_ptr,
...
//************************************************************************************
gl33_line_render_2d_plug::gl33_line_render_2d_plug( this_rref_t rhv ) : this_base_t( std::move(rhv) )
{
    ...
    so_move_member_ptr( _line_info_ptr, rhv ) ;

    _per_group_datas = std::move( rhv._per_group_datas ) ;
}

//************************************************************************************
gl33_line_render_2d_plug::~gl33_line_render_2d_plug( void_t )
{
    this_t::vb_t::destroy( _vb ) ;
    this_t::ib_t::destroy( _ib ) ;
    this_t::data_buffer_t::destroy( _line_info_ptr ) ;
...
//************************************************************************************
gl33_line_render_2d_plug::this_ptr_t gl33_line_render_2d_plug::create( this_rref_t rhv, so_memory::purpose_cref_t p )
{
    return so_gfx::memory::alloc( std::move( rhv ), p ) ;
}

//************************************************************************************
void_t gl33_line_render_2d_plug::destroy( this_ptr_t ptr )
{
    so_gfx::memory::dealloc( ptr ) ;
}

//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_load( load_info_cref_t li )
{
    if( li.reload )
    {
        so_log::global_t::warning( "[gl33_line_render_2d_plug::on_load] : reload not supported" ) ;
        return so_gpx::plug_result::failed ;
    }

    so_std::string_t const base( so_std::string_t( GFX_SOURCE_DIR ) + "/line/gl33/shader" ) ;

    // text render shader
    {
        so_std::string_t const vs_path = base + so_std::string( "/line_render_2d.vs.glsl" ) ;
        so_std::string_t const ps_path = base + so_std::string( "/line_render_2d.ps.glsl" ) ;
...

        {
            ...
            vsh.wait_for_operation( [&] ( char_cptr_t din, size_t sib, so_io::result res )
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_line_render_2d_plug::on_load] : Vertex shader not found : " + vs_path ) ;
...

        {
            ...
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_line_render_2d_plug::on_load] : Pixel shader not found : " + ps_path ) ;
...

    {
        ...
        size_t const num_lines = _num_lines ;

        for( size_t i = 0; i < num_lines; ++i )
        {
            _vb->add_vertex( vertex_t { so_math::vec3f_t( 0.0f, 0.0f, 0.0f ) } ) ;
            _vb->add_vertex( vertex_t { so_math::vec3f_t( 0.0f, 0.0f, 0.0f ) } ) ;
        }

        for( size_t i = 0; i < num_lines; ++i )
        {
            uint_t const index = uint_t( i ) << 1 ;
    ...
    {
        data_buffer_t db ;
        _line_info_ptr = data_buffer_t::create( std::move( db ),
            "[gl33_line_render_2d_plug::on_load] : line info" ) ;
    }
...
//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_unload( void_t )
{
    return so_gpx::plug_result::ok ;
}

//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_initialize( init_info_cref_t )
{
    {
        {
        ...
        }
        {
            ...
            _config->bind( so_gpu::primitive_type::lines, _vb, _ib ) ;
        }
    }

    {
        auto const res = this_t::api()->create_buffer( _line_info_ptr ) ;
    }
...
//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_release( void_t )
{
    ...
    this_t::api()->release_buffer( _line_info_ptr ) ;
...
//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_update( update_info_cref_t )
{
    // 1. copy the data
    {
        _line_info_ptr->resize(0) ;

        // copy all glyph data
        {
            for( auto const & gi : _sd->line_infos )
            {
                so_math::vec4f_t const color = gi.color ;
                so_math::vec2f_t const pos0 = gi.pos0 ;
                so_math::vec2f_t const pos1 = gi.pos1 ;

                _line_info_ptr->add_element( so_math::vec4f_t( pos0.x(), pos0.y(), pos1.x(), pos1.y() ) ) ;
                _line_info_ptr->add_element( color ) ;
            }
        }
            ...
            for( auto const & gi : _sd->per_group_infos )
            {
                ...
                if( iter == _per_group_datas.end() )
                {
                    ...
                    // configure variable set
                    {
                        ...
                        var_set->bind_buffer<so_math::vec4f_t>( "u_line_info", _line_info_ptr ) ;
    ...
    {
        auto const res = this_t::api()->alloc_buffer_memory( _line_info_ptr,
...
//************************************************************************************
so_gpx::plug_result gl33_line_render_2d_plug::on_execute( execute_info_cref_t ri )
{
    auto iter = std::find_if( _per_group_datas.begin(), _per_group_datas.end(), 
...
//************************************************************************************
void_t gl33_line_render_2d_plug::destroy( void_t )
{
    this_t::destroy( this ) ;
}
