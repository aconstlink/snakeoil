...
{
    return so_var::memory::alloc( this_t(), p ) ;
}
...
//*************************************************************************************
so_var::result variable_set::add( so_var::key_in_t key, so_var::ivariable_ptr_t var_ptr )
{
    if( so_core::is_nullptr(var_ptr) )
        return so_var::invalid_argument ;
...
//*************************************************************************************
so_var::result variable_set::remove( so_var::key_in_t key )
{
    auto iter = std::find_if( _variables.begin(), _variables.end(),
        [&]( this_t::store_data_cref_t item )
    {
...
//*************************************************************************************
bool_t variable_set::get_variable( so_var::key_in_t key, so_var::ivariable_ptr_t & ptr_out )
{
    auto iter = std::find_if( _variables.begin(), _variables.end(),
        [&]( this_t::store_data_cref_t item )
    {
