...
//------------------------------------------------------------
#ifndef _SNAKEOIL_COLLIDE_3D_HIT_TEST_BOX_RAY_HPP_
#define _SNAKEOIL_COLLIDE_3D_HIT_TEST_BOX_RAY_HPP_
...
namespace so_collide 
{
    namespace so_3d
    {
		...
		template< typename type_t >
		static hit_test_type hit_test_box_ray( aabb< type_t > const & box, so_math::ray3f_t const & ray )
		{
			typedef vector3<type_t> vec3_t ;

			float t_min = std::numeric_limits<type_t>::min() ;
			float t_max = std::numeric_limits<type_t>::max() ;

			vec3_t p = box.get_center() - ray.get_origin() ;
			vec3_t h = box.get_max() - box.get_center() ;
        ...
        template< typename type_t >
        static hit_test_type hit_test_box_ray( aabb< type_t > const & box, so_math::ray3f_t const & ray, type_t & t_out )
        {
			typedef vector3<type_t> vec3_t ;
        ...
        template< typename type_t >
        static bool_t intersect( aabb< type_t > const & box, so_math::ray3f_t const & ray, type_t & t_out )
        {
