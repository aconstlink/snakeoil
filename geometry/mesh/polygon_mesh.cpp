...
//*******************************************************************
polygon_mesh::this_ptr_t polygon_mesh::create( so_memory::purpose_cref_t purp )
{
    return so_geo::memory::alloc( this_t(), purp ) ;
}

//*******************************************************************
polygon_mesh::this_ptr_t polygon_mesh::create( this_cref_t rhv, so_memory::purpose_cref_t purp ) 
{
    return so_geo::memory::alloc( rhv, purp ) ;
}

//*******************************************************************
polygon_mesh::this_ptr_t polygon_mesh::create( this_rref_t rhv, so_memory::purpose_cref_t purp )
{
    return so_geo::memory::alloc( std::move(rhv), purp ) ;
}

//*******************************************************************
void_t polygon_mesh::destroy( this_ptr_t ptr )
{
...
//*******************************************************************
so_geo::result polygon_mesh::flatten( tri_mesh_ref_t mesh_out ) const
{
    tri_mesh the_mesh ;
...
//*******************************************************************
so_geo::result polygon_mesh::flatten( flat_tri_mesh_ref_t mesh_out ) const
{
    tri_mesh_t mesh_a ;
...
//*******************************************************************
void_t polygon_mesh::destroy( void_t ) 
{
    this_t::destroy( this ) ;
}
...
polygon_mesh::this_t polygon_mesh::repeat( size_t const times ) const 
{
    ...
    // repeat polygons
    {
        so_geo::uints_t old_polys = std::move( m.polygons ) ;
        so_geo::uints_t new_polys( times * old_polys.size() ) ;
    ...
    {
        so_geo::uints_t old_indices = std::move( m.indices ) ;
        so_geo::uints_t new_indices( times * old_indices.size() ) ;
    ...
    {
        so_geo::uints_t old_indices = std::move( m.normals_indices[k] ) ;
        so_geo::uints_t new_indices( times * old_indices.size() ) ;
    ...
    {
        so_geo::uints_t old_indices = std::move( m.texcoords_indices[k] ) ;
        so_geo::uints_t new_indices( times * old_indices.size() ) ;
...
polygon_mesh::this_t polygon_mesh::repeat_full( size_t const times ) const 
{
    ...
    {
        so_geo::uints_t old_indices = std::move( m.indices ) ;
        so_geo::floats_t old_pos = std::move( m.positions ) ;

        so_geo::uints_t new_indices( times * old_indices.size() ) ;
        so_geo::floats_t new_pos( times * old_pos.size() ) ;

        size_t const div = so_geo::vector_component_format_to_number(m.position_format) ;
    ...
    {
        so_geo::uints_t old_indices = std::move( m.normals_indices[k] ) ;
        so_geo::floats_t old_normals = std::move( m.normals[k] ) ;

        so_geo::uints_t new_indices( times * old_indices.size() ) ;
        so_geo::floats_t new_normals( times * old_normals.size() ) ;

        size_t const div = so_geo::vector_component_format_to_number(m.normal_format) ;
    ...
    {
        so_geo::uints_t old_indices = std::move( m.texcoords_indices[k] ) ;
        so_geo::floats_t old_texcoords = std::move( m.texcoords[k] ) ;

        so_geo::uints_t new_indices( times * old_indices.size() ) ;
        so_geo::floats_t new_texcoords( times * old_texcoords.size() ) ;

        size_t const div = so_geo::texcoords_component_format_to_number(m.texcoord_format) ;
