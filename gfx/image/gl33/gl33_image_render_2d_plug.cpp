...
//*********************************************************************
so_gpx::plug_result gl33_image_render_2d_plug::on_load( load_info_cref_t li )
{
    if( li.reload )
    {
        so_log::global_t::warning( "[gl33_image_render_2d_plug::on_load] : reload not supported" ) ;
        return so_gpx::plug_result::failed ;
    }

    auto const base = so_gfx::find_shader_path( "/image/gl33/shader" ) ;

    // text render shader
    {
        auto const vs_path = base / so_io::path_t( "/image_render_2d.vs.glsl" ) ;
        auto const ps_path = base / so_io::path_t( "/image_render_2d.ps.glsl" ) ;
...

        {
            ...
            vsh.wait_for_operation( [&] ( char_cptr_t din, size_t sib, so_io::result res )
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_image_render_2d_plug::on_load] : Vertex shader not found : " + vs_path.string() ) ;
...

        {
            ...
            {
                so_log::global::error_and_exit( so_io::no_success( res ),
                    "[gl33_image_render_2d_plug::on_load] : Pixel shader not found : " + ps_path.string() ) ;
